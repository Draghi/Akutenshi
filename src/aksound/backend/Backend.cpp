...
#include <aksound/backend/internal/Mal.hpp>
#include <aksound/backend/Util.hpp>
#include <mini_al.h>
#include <algorithm>
#include <atomic>
#include <memory>
...
std::optional<ContextInfo> aks::backend::getContextInfo() {
	...
	switch(malContext.backend) {
		...
		case mal_backend::mal_backend_audio4:     return ContextInfo{"Audio4"};
...
static bool initDevice(mal_context* context, mal_device* device, const DeviceIdentifier& deviceID, StreamFormat streamFormat, const std::function<upload_callback_f>& callback) {
	auto bufferFrames = mal_get_default_buffer_size_in_frames(mal_performance_profile::mal_performance_profile_low_latency, streamFormat.sampleRate);
	...
	mal_device_config cfg{
		...
		0,                                          // mal_uint32 bufferSizeInMilliseconds
