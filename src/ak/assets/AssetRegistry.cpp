...
bool AssetRegistry::proccessFile(const akfs::Path& path) {
	...
	auto assetInfo = akd::tryDeserialize<akas::AssetInfo>(akd::fromJsonFile(path));
	if (!assetInfo) {
...
std::optional<std::pair<akas::AssetInfo, akfs::Path>> AssetRegistry::tryGetAssetInfoBySUID(const akd::SUID& suid) const {
	auto iter = m_assetBySUID.find(suid);
	if (iter == m_assetBySUID.end()) return {};
	return {m_assetInfo[iter->second]};
}

std::optional<std::pair<akas::AssetInfo, akfs::Path>> AssetRegistry::tryGetAssetInfoBySource(const akfs::Path& path) const {
	auto iter = m_assetBySource.find(path);
	if (iter == m_assetBySource.end()) return {};
	return {m_assetInfo[iter->second]};
}

std::optional<std::pair<akas::AssetInfo, akfs::Path>> AssetRegistry::tryGetAssetInfoByDestination(const akfs::Path& path) const {
