...
static akas::Primitive processPrimitive(const Asset& asset, const Primitive& primitive, const std::vector<akas::ConversionInfo>& materialUIDs);
static std::vector<akas::VertexSurfaceData> extractSurfaceData(const Asset& asset, const Primitive& primitive);
...
static std::vector<akas::VertexWeightData> extractSkinningData(const Asset& asset, const Primitive& primitive);
...
static std::vector<akas::VertexSurfaceData> processSurfaceData(const std::vector<akm::Vec3>& positions, const std::vector<akm::Vec3>& tangents, const std::vector<akm::Vec3>& bitangents, std::vector<akm::Vec3> normals);
static std::vector<std::map<uint32, fpSingle>> collectBoneWeights(const Asset& asset, const Primitive& primitive);
static akas::VertexWeightData selectHighestWeightedBones(const std::map<uint32, fpSingle>& boneWeights);

akas::Mesh akas::gltf::processGLTFMesh(const Asset& asset, const Mesh& mesh, const std::vector<akas::ConversionInfo>& materialUIDs) {
	std::vector<akas::Primitive> primitives;
...
static std::vector<akas::VertexSurfaceData> processSurfaceData(const std::vector<akm::Vec3>& positions, const std::vector<akm::Vec3>& tangents, const std::vector<akm::Vec3>& bitangents, std::vector<akm::Vec3> normals) {
	std::vector<akas::VertexSurfaceData> result; result.reserve(positions.size());
...
static std::vector<akm::Vec4> extractColourData(const Asset& asset, const Primitive& primitive) {
	if (primitive.colourIDs.size() > 0) {
		...
		if (asset.accessors[primitive.colourIDs[0]].type == AccessorType::Vec3) return ak::convert_to<std::vector<akm::Vec4>>(extractAccessorData<akm::Vec3>(asset, primitive.colourIDs[0]), [](const auto& colourRGB){ return akm::Vec4{colourRGB.r, colourRGB.g, colourRGB.b, 1.0f};});
...
static std::vector<akas::VertexWeightData> extractSkinningData(const Asset& asset, const Primitive& primitive) {
	if (primitive.jointIDs.size() > 0) {
		...
		std::vector<akas::VertexWeightData> result; result.reserve(vertexWeights.size());
		for(auto& boneWeights : vertexWeights) result.push_back(selectHighestWeightedBones(boneWeights));
		return result;
	}
	return std::vector<akas::VertexWeightData>();
}
