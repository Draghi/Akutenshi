...
static bool convertImage(ConversionHelper& state, const akfs::Path& cfgPath, akd::PValue& cfg) { return convertCopyOnly<&aka::ConversionHelper::registerImage>("image", state, cfgPath, cfg); }
static bool convertShaderStage(ConversionHelper& state, const akfs::Path& cfgPath, akd::PValue& cfg) { return convertCopyOnly<&aka::ConversionHelper::registerShaderStage>("shader stage", state, cfgPath, cfg); }
static bool convertSound(ConversionHelper& state, const akfs::Path& cfgPath, akd::PValue& cfg) { return convertCopyOnly<&aka::ConversionHelper::registerSound>("sound", state, cfgPath, cfg); }

static auto writeAssetMetaFile = [](const akfs::Path& dst, const akd::SUID& suid, aka::AssetType assetType, const std::string& name, const akfs::Path& source){
	akd::PValue dstData; akd::serialize(dstData, aka::AssetInfo{suid, assetType, name, source});
...
void aka::convertDirectory(const akfs::Path& dir) {
	...
	std::map<aka::AssetSourceType, std::vector<akfs::Path>> collectedAssets;
	akfs::iterateDirectory(dir, [&](const akfs::Path& path, bool isDir){
		...
		collectedAssets[akd::deserialize<aka::AssetSourceType>(convData["type"])].push_back(path);
		fileCount     += 1;
		return true;
	}, true);
...
static void writeMeshes(ConversionHelper& state) {
	for(auto& mesh : state.meshes()) {
		if (!writeAssetMetaFile(mesh.first.destination, mesh.first.identifier, aka::AssetType::Mesh, mesh.first.displayName, {})) continue;
...
static void writeMaterials(ConversionHelper& state) {
	for(auto& material : state.materials()) {
		if (!writeAssetMetaFile(material.first.destination, material.first.identifier, aka::AssetType::Material, material.first.displayName, {})) continue;
...
static void writeImages(ConversionHelper& state) {
	for(auto& copy : state.images()) {
		if (!writeAssetMetaFile(copy.first.destination, copy.first.identifier, aka::AssetType::Image, copy.first.displayName, copy.second)) continue;
...
static void writeShaderStages(ConversionHelper& state) {
	for(auto& copy : state.shaderStages()) {
		if (!writeAssetMetaFile(copy.first.destination, copy.first.identifier, aka::AssetType::ShaderStage, copy.first.displayName, copy.second)) continue;
		if (!akfs::copy(copy.second, akfs::Path(copy.first.destination).clearExtension())) continue;
	}
}
...
static void writeShaderPrograms(ConversionHelper& state) {
	for(auto& shader : state.shaderPrograms()) {
		if (!writeAssetMetaFile(shader.first.destination, shader.first.identifier, aka::AssetType::ShaderProgram, shader.first.displayName, {})) continue;
...
static void writeAnimations(ConversionHelper& state) {
	for(auto& animation : state.animations()) {
		if (!writeAssetMetaFile(animation.first.destination, animation.first.identifier, aka::AssetType::Animation, animation.first.displayName, {})) continue;
...
static void writeTextures(ConversionHelper& state) {
	for(auto& texture : state.textures()) {
		if (!writeAssetMetaFile(texture.first.destination, texture.first.identifier, aka::AssetType::Texture, texture.first.displayName, {})) continue;
...
static void writeSounds(ConversionHelper& state) {
	for(auto& copy : state.sounds()) {
		if (!writeAssetMetaFile(copy.first.destination, copy.first.identifier, aka::AssetType::Sound, copy.first.displayName, copy.second)) continue;
		if (!akfs::copy(copy.second, akfs::Path(copy.first.destination).clearExtension())) continue;
	}
}
...
static bool convertTexture(ConversionHelper& convertHelper, const akfs::Path& cfgPath, akd::PValue& cfg) {
	...
	auto result = akd::deserialize<aka::Texture>(texCfg);
...
static bool convertShaderProgram(ConversionHelper& conversionHelper, const akfs::Path& cfgPath, akd::PValue& cfg) {
	...
	auto result = akd::deserialize<aka::ShaderProgram>(shaderCfg);
